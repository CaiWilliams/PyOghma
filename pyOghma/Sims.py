...
class Sims:
    ...
    def find_file(self, file):
        if self.json_name == self.icon.replace("_", "") or self.json_name.replace("_", "") == self.icon:
            config_dir = config_dir = os.path.abspath(os.path.join('pyOghma','Sim_Defaults', '*', 'configs', self.json_name, file))
        else:
            config_dir = os.path.abspath(os.path.join('pyOghma','Sim_Defaults', '*', 'configs', self.json_name, self.name.lower().replace("\n","_"), file))
...
class TimeDomainMesh:
    ...
    def find_file(self, file):
        config_dir = os.path.abspath(os.path.join('pyOghma','Sim_Defaults', '*', 'configs', 'time_domain', file))
        filename = glob(config_dir)[0]
        self.loaded_filename = str(filename)
        return str(filename)
...
class FrequencyDomainSegment:
    ...
    def find_file(self, file):
        config_dir = config_dir = os.path.abspath(os.path.join('pyOghma','Sim_Defaults', '*', 'configs', 'fx_domain', file))
        filename = glob(config_dir)[0]
        self.loaded_filename = str(filename)
        return str(filename)
